...
Indeed, the Open Science Collaboration (OSC) (2015) argued, "There is no single standard for evaluating replication success," and then analyzed replicate pairs in a variety of ways.

Despite immediate criticism of the analyses (see Gilbert, Pettigrew, and King, 2015), the core set of OSC analyses has become something of a standard in replication research.
...
Schweinsberg, et al. (2016), while conducting several---i.e., more than one---replicates of the same findings, aggregated replicate results so that they could use the same analyses as the OSC.
...
Given their prevalence in the replication literature, the analyses conducted by the OSC have a few important aspects worth exploring.
...
In this paper, we explore methods for assessing replication as proposed by the OSC.
...
For programs such as that of the OSC, this invovles $\theta_1$ for the original study and $\theta_2$ for the replicate.
...
One metric used by the OSC to determine if studies successfully replicated to to check that both the original and replicate study corresponded in sign and statistical significance.
That is, findings were deemed to have replcated if both studies found significant effects in the same direction---i.e., both positive---or if both studies found null effects.
This type of metric would seem to answer an important question about replication, particularly of published research which more often involves statistically significant findings.
Indeed, Fisher (1935) noted that "we may say that a phenomenon is experimentally demonstrable when we know how to conduct an experiment which will rarely fail to give us statistically significant results."
...
Thus, it is entirely possible that $\theta_1 = \theta_2 > 0$, so that the studies replicate exactly, but only $T_1$ is statistically significant.
...
Thus, the type I error rate is given by
<!-- \begin{align} -->
<!-- \begin{split} -->
...
& + \Phi\left(1.96 - \theta/\sqrt{v_1}\right) [1 - \Phi\left(1.96 - \theta/\sqrt{v_2}\right)]\\ 
& + \Phi\left(-1.96 - \theta/\sqrt{v_1}\right) [1 - \Phi\left(-1.96 - \theta/\sqrt{v_2}\right)]\\
& + [1 - \Phi\left(-1.96 - \theta/\sqrt{v_1}\right)] \Phi\left(-1.96 - \theta/\sqrt{v_2}\right)
\end{align*}
\noindent
...
In Figure \ref{fig:sig:falsepos}, we see that the false positive rates are nearly all above the nominal 5\%. 
...
Thus, unless study 1 has at least 95\% power, determining if studies replicated according to correspondence of sign and statisitical significance can result in type I error rates well above 5\%.
...
Conversely, if we assume $\theta_1 \neq \theta_2$, then we would want the test to reject the null hypothesis frequently---that is, we would want a low type II error rate.
To conclude that the studies do not replicate would require one finding to be significant and in one direction (e.g., positive), and the other to be either nonsignificant or significant but in the opposite direction (e.g., negative).
...
For instance, if study 1 has power 80\%, then this test would conclude that the studies do not replicate 24\% of the time. 
However, if the power were lower, say 60\%, then the test would make this error 43\% of the time.

If both of the results are nonzero but the same direction (e.g., both positive), then the error rate will be large whenever both studies have high (or low) power. 
...
Conversely, if the studies involve very different effect parameters, the test will often conclude that studes replicate. 
